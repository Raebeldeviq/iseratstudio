...
export const REGRESSION_85_SCOPE_FORMAT = 1;
...
function normalizedPortalStatuses(values) {
  const records = (Array.isArray(values) ? values : []).map((value) => ({
    externalObjectNumber: clean(value?.externalObjectNumber, 40),
    status: clean(value?.status, 100),
    observedAt: iso(value?.observedAt),
    channel: clean(value?.channel || "immoprofessional-portal-read-only", 100),
  }));
  const identifiers = records.map((record) => record.externalObjectNumber);
  if (records.length !== REGRESSION_85_EXPECTED_COUNT || new Set(identifiers).size !== records.length) {
    throw scopeError(
      "REGRESSION_85_PORTAL_SNAPSHOT_INCOMPLETE",
      "Der read-only Portalstatus muss exakt je einer der 85 Objektnummern dokumentiert sein.",
    );
  }
  return new Map(records.map((record) => [record.externalObjectNumber, record]));
}
 ...
 * ausschließt.
 */
export function deriveRegression85Scope(state, uploadEvents, uploadLedger, options = {}) {
  ...
  const portalByExternalId = normalizedPortalStatuses(options.portalStatuses);
  ...
  const items = [...uniqueJobs.values()].map((event) => {
    ...
    if (listing?.status !== WORKFLOW_STATUS.TRANSFERRED_PENDING_IMPORT) reasons.push("catalog_status_changed");
    ...
    const portal = portalByExternalId.get(listing?.externalId);
    if (!portal) reasons.push("portal_status_missing");
    if (reasons.length) {
      throw scopeError(
        "REGRESSION_85_SCOPE_MISMATCH",
    ...
    return {
      ...
      portalStatusObservedAt: portal.observedAt,
      portalStatusChannel: portal.channel,
    };
  }).sort((left, right) =>
  ...
  const scope = {
    ...
    contract: "regression-85-exact-allowlist-v1",
  ...
  return { scope, scopeHash: hashRegression85Scope(scope) };
}
...
export function assertRegression85Campaign(value) {
  ...
  if (!/^[a-f0-9]{64}$/u.test(scopeHash) || hashRegression85Scope(scope) !== scopeHash) reasons.push("scope_hash_invalid");
...
function initialCampaign(scope, scopeHash, input = {}) {
...
export function createRegression85CampaignStore(path) {
  return {
    ...
    async initialize(scope, scopeHash, input = {}) {
      const release = await acquireMutationLock(path);
      try {
        const current = await readCampaign(path);
        if (current) {
          if (current.scopeHash !== scopeHash) throw scopeError("REGRESSION_85_SCOPE_IMMUTABLE", "Eine abweichende 85er-Allowlist darf die persistierte Kampagne nicht ersetzen.");
          return { ...current, valid: true, fallbackReason: "", idempotent: true };
        }
        const campaign = assertRegression85Campaign(initialCampaign(scope, scopeHash, input));
    ...
    async update(mutator, input = {}) {
      ...
      try {
        ...
        if (next.scopeHash !== current.scopeHash || canonicalJson(next.scope) !== canonicalJson(current.scope)) {

...
export const HOUSE_PRICE_CATALOG = Object.freeze(
  Object.fromEntries(PRICE_ENTRIES.map((entry) => [entry.key, Object.freeze({ ...entry })])),
);
...
/**
 * Resolves a price from house names, cover filenames and floorplan filenames.
 * Image versions such as V2 or V5 are intentionally ignored; L and XL remain
 * distinct house models. Ambiguous unsuffixed models are never guessed.
 */
export function resolveHousePrice(values) {
  const parsed = (Array.isArray(values) ? values : [values]).flatMap(identifiers);
  if (!parsed.length) return null;

  const exactKeys = [...new Set(parsed
    .map((identifier) => identifier.key)
    .filter((key) => HOUSE_PRICE_CATALOG[key]))];
  ...
  const entries = PRICE_ENTRIES.filter((entry) => entry.key.replace(/(?:XL|L)$/, "") === modelKeys[0]);

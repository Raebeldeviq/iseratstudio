...
async function scanFiles(root, output, source) {
  ...
  try {
    ...
    if (error && typeof error === "object" && (error.killed || error.signal)) {
      throw new Error(
        "macOS hat den Zugriff auf iCloud Drive nicht rechtzeitig freigegeben. Bitte Terminal unter Systemeinstellungen > Datenschutz & Sicherheit > Dateien und Ordner den Zugriff auf iCloud Drive erlauben.",
      );
    }
    throw error;
  }
...
export async function getMediaLibraryItem(id, root, interiorRoot) {
  const items = await indexMediaLibrary(root, interiorRoot);
  return items.find((item) => item.id === String(id || "")) || null;
}

export async function recommendedMediaSequence(coverId, root, interiorRoot) {
  const items = await indexMediaLibrary(root, interiorRoot);

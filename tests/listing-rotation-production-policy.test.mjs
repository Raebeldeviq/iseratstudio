...
test("missing, damaged and unknown production policies fail closed", async () => {
  ...
  assert.equal(normalizeListingRotationProductionPolicy({ format: 2, maxRunItems: 4, startupCatchupMode: "guarded", expectedRuntimeCommit: RUNTIME_COMMIT }).valid, false);
...
test("policy persists an explicit cap of three and a bounded startup mode", async () => {
  ...
  const saved = await store.save({ maxRunItems: 3, startupCatchupMode: "detect-only", expectedRuntimeCommit: RUNTIME_COMMIT });
  ...
  const guarded = await store.save({ maxRunItems: 3, startupCatchupMode: "guarded", expectedRuntimeCommit: RUNTIME_COMMIT });

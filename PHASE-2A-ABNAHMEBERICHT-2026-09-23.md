...
- Read-only-Bestandsklassifizierung ist über `phase2b-claim-scan.mjs` und `phase2b-claim-scan-cli.mjs` vorbereitet. Sie verwendet dieselbe zentrale Claim-Policy und besitzt keinen Speicher-, Upload- oder Textgenerierungspfad. Der Bericht weist jeden blockierenden Claim zusätzlich mit Position, Begründung, vorhandener und fehlender Evidenz sowie dem nur soweit gespeicherten Textursprung aus.

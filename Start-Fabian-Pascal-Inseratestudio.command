...
SESSION_FILE="$DATA_ROOT/helper-session"
...
if ! /usr/bin/swift --version >/dev/null 2>&1; then
  print -u2 "Die Apple Command Line Tools mit Swift wurden nicht gefunden. Bitte zuerst 'xcode-select --install' ausführen."
  read -k 1 "?Taste drücken zum Schließen …"
  exit 1
fi
...
if [[ ! -s "$SESSION_FILE" ]]; then
  umask 077
  /usr/bin/openssl rand -hex 32 > "$SESSION_FILE"
fi

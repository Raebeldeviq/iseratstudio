...
export function inferImageRole({ filename = "", relativePath = "", kind = "", role = "", isFloorplan = false } = {}) {
  ...
  if (kind === "floorplan" || isFloorplan || /grundriss/.test(text)) {
    ...
    if (level === "ground") return "floorplan_ground";
  ...
  if (/ausgezeichnet gebaut|hausbau design award|musterhaus report/.test(text)) return "awards";
  ...
  if (/entwicklungsraum|fur die kleinen|kinderzimmer|kinder|kids/.test(text)) return "kids";
  if (/setz dich und ruh dich aus|wohnzimmer|living|lounge/.test(text)) return "living";
  if (/work life balance|arbeitszimmer|buro|office/.test(text)) return "office";
  if (kind === "house") return "cover";
  return "other";
}

export function captionForImageRole(role, filename = "", fallback = "") {
  if (role === "cover") return TITLE_IMAGE_CAPTIONS.includes(fallback)
    ? fallback
    : TITLE_IMAGE_CAPTIONS[0];
  if (role === "floorplan_upper" && floorplanLevel(filename) === "attic") return "Dein Dachgeschoss";
...
/**
 * Sorts the fixed sequence blocks while preserving the user's chosen order
 * inside the six-image interior block.
 * @template T
 * @param {readonly T[]} images
 * @returns {T[]}
 */
...
function mediaPreference(left, right) {
  ...
  const rootPenalty = (item) => item.collection === "Inneneinrichtung" ? 0 : 1;
  const difference = copyPenalty(left) - copyPenalty(right) || rootPenalty(left) - rootPenalty(right);
  if (difference) return difference;
  return String(left.relativePath || left.filename).localeCompare(String(right.relativePath || right.filename), "de", { numeric: true });
}
...
function matchingFloorplans(cover, items) {
  ...
  if (!coverVariant) {
    return { items: [], warnings: ["Haustyp und Version konnten aus dem Titelbild nicht erkannt werden."] };
  }
  const matchingRoof = coverVariant.roof || KNOWN_ROOF_BY_VARIANT[`${coverVariant.modelKey}:${coverVariant.version}`] || "";
  let usedVersionlessFallback = false;
  let candidates = items
    .filter((item) => item.kind === "floorplan")
    .map((item) => ({ item, variant: parseHouseVariant(`${item.filename} ${item.relativePath || ""}`) }))
  ...
  if (!matchingRoof && (!coverVariant.version || usedVersionlessFallback) && roofTypes.size > 1) {
    return {
      items: [],
      warnings: [`Für ${coverVariant.family} ${coverVariant.model} existieren mehrere Dachvarianten. Bitte eine eindeutig bezeichnete Hausansicht wählen.`],
    };
  }
  ...
  if (byLevel.attic[0]) selected.push({
    ...byLevel.attic[0],
  ...
  if (!selected.some((item) => item.role === "floorplan_ground")) {
    warnings.push(`Kein eindeutiger Erdgeschoss-Grundriss für ${coverVariant.family} ${coverVariant.model} ${coverVariant.version || coverVariant.roof} gefunden.`.trim());
  }
  const isSolutionBungalow = coverVariant.family === "SOL" && ["82", "101", "107", "110"].includes(coverVariant.model);
  if (!isSolutionBungalow && !selected.some((item) => item.role === "floorplan_upper")) {
    warnings.push(`Kein eindeutiger Ober- oder Dachgeschoss-Grundriss für ${coverVariant.family} ${coverVariant.model} ${coverVariant.version || coverVariant.roof} gefunden.`.trim());
  }
...
export function buildRecommendedMediaSequence(coverId, items) {
  ...
  if (cover.brandedCover === false) {
    return { items: [], warnings: ["Bitte die PNG-Hausansicht mit LivingHaus-Logo und eindeutiger SUN-/SOL-Version auswählen."] };
  }
  ...
  else warnings.push("Der emotionale Catch „Hier beginnt dein Zuhause“ fehlt.");
...
export function imageSequenceIssues(images, {
  ...
  if (!images.some((image) => IMAGE_ROLE_VALUES.includes(image.role) && !["promotion", "other"].includes(image.role))) return [];
  ...
  for (const role of INTERIOR_IMAGE_ROLES) {
    const count = roles.filter((value) => value === role).length;
    if (count > 1 && !issues.some((issue) => issue.startsWith(IMAGE_ROLE_LABELS[role]))) {
      issues.push(`${IMAGE_ROLE_LABELS[role]} ist ${count}-mal vorhanden.`);
    }
  }
  if (images.length > maximumImages) issues.push(`Die Bildfolge enthält ${images.length} statt maximal ${maximumImages} Bilder.`);
  const ranks = images
